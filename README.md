...
DreamFlow attributes real DreamDEX Event Contract orders to the acquisition channel that generated them, verifies that attribution directly from on-chain events, and activates native builder monetization whenever the live pool supports DreamDEX Builder Codes.

> **Clicks are not conversions. DreamFlow attributes actual orders and fills.**
...
But once those applications start acquiring users, they face a simple problem:

> **Which channel actually created real DreamDEX activity?**

Traditional analytics can prove that somebody clicked a link.

They cannot independently prove that the user actually placed an Event Contract order or received a fill.

DreamFlow moves attribution into the trading transaction itself.

Every acquisition channel gets a campaign ID.
...
When a user trades through that channel, DreamFlow writes the campaign ID into DreamDEX's native:
...
The resulting transaction can then be decoded directly from DreamDEX's on-chain `OrderPlaced` event.
...
No private analytics database is required to prove the attribution.

Anyone can verify it from the transaction.

---

# Why DreamFlow?

Most prediction-market products focus on:
...
DreamFlow focuses on a different layer:

> **Where did real trading activity come from, and how can the application that generated it monetize that flow?**

That makes DreamFlow complementary to DreamDEX frontends, trading agents, games, wallets, and other applications rather than another competing trading interface.

---

# Core Features

## 1. On-Chain Campaign Attribution

Every DreamFlow campaign has a simple `uint64` identifier.
